...
pub fn main() {
    emerald::start(Box::new(HelloWolrd), GameSettings::default())
}

pub struct ElapsedTime(f32);

pub struct HelloWolrd;

impl Game for HelloWolrd {
    fn initialize(&mut self, mut emd: Emerald) {
        let font = emd
            .loader()
            .font("./src/Roboto-Light.ttf", 40)
            .unwrap();

        let mut left_aligned_label = Label::new("Hello Smoke Installer!", font.clone(), 40);
